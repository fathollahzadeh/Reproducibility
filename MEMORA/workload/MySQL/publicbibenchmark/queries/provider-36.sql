SELECT
    ...
    CAST(provider6.npi AS SIGNED) AS npi,
    SUM(
        (
            CAST(
                provider6.averagesubmittedchrgamt AS DOUBLE PRECISION
            ) * CAST(
                CAST(provider6.benedaysrvccnt AS SIGNED) AS DOUBLE PRECISION
            )
        )
    ...
    SUM(
        (
            CAST(
                ...
                CAST(provider6.linesrvccnt AS SIGNED) AS DOUBLE PRECISION
            )
        )
