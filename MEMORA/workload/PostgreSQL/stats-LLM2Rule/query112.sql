...
FROM (SELECT "$cor2"."postid0", "$cor2"."$f4" * "t5"."EXPR$0" AS "$f4"
...
FROM "postlinks"
WHERE "linktypeid" = 1 AND "creationdate" >= TIMESTAMP '2011-06-14 13:31:35'
GROUP BY "postid"
HAVING "postid" = "$cor3"."postid") AS "t2") AS "$cor2",
LATERAL (SELECT "postid", COUNT(*) AS "EXPR$0"
FROM "posthistory"
GROUP BY "postid"
HAVING "$cor2"."postid0" = "postid") AS "t5") AS "$cor1",
LATERAL (SELECT "postid", COUNT(*) AS "EXPR$0"
FROM "votes"
