...
FROM "users"
WHERE "views" = 5 AND "downvotes" >= 0 AND ("upvotes" >= 0 AND "upvotes" <= 224) AND "creationdate" <= TIMESTAMP '2014-09-04 04:41:22'
GROUP BY "id"
HAVING "$cor1"."userid" = "id") AS "t6") AS "$cor0",
LATERAL (SELECT "userid", COUNT(*) AS "EXPR$0"
FROM "badges"
WHERE "date" >= TIMESTAMP '2010-07-19 19:39:10' AND "date" <= TIMESTAMP '2014-09-05 18:37:48'
GROUP BY "userid"
