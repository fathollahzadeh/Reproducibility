SELECT COALESCE(SUM("$cor0"."$f4" * "t14"."EXPR$0"), 0)
FROM (SELECT "$cor1"."userid" AS "userid0", "$cor1"."$f4" * "t10"."EXPR$0" AS "$f4"
FROM (SELECT "t7"."userid", "$cor2"."$f4" * "t7"."EXPR$0" AS "$f4"
FROM (SELECT "t3"."owneruserid", "$cor3"."EXPR$0" * "t3"."EXPR$0" AS "$f4"
FROM (SELECT "userid", COUNT(*) AS "EXPR$0"
FROM "comments"
WHERE "score" = 0
GROUP BY "userid") AS "$cor3",
LATERAL (SELECT "owneruserid", COUNT(*) AS "EXPR$0"
FROM "posts"
WHERE "score" >= -2 AND ("commentcount" >= 0 AND "commentcount" <= 12) AND ("favoritecount" >= 0 AND "favoritecount" <= 6)
GROUP BY "owneruserid"
HAVING "owneruserid" = "$cor3"."userid") AS "t3") AS "$cor2",
LATERAL (SELECT "userid", COUNT(*) AS "EXPR$0"
FROM "posthistory"
WHERE "creationdate" <= TIMESTAMP '2014-08-18 08:54:12'
GROUP BY "userid"
HAVING "$cor2"."owneruserid" = "userid") AS "t7") AS "$cor1",
LATERAL (SELECT "userid", COUNT(*) AS "EXPR$0"
FROM "badges"
GROUP BY "userid"
HAVING "$cor1"."userid" = "userid") AS "t10") AS "$cor0",
LATERAL (SELECT "id", COUNT(*) AS "EXPR$0"
FROM "users"
WHERE "views" = 0 AND ("downvotes" >= 0 AND "downvotes" <= 60)
GROUP BY "id"
HAVING "$cor0"."userid0" = "id") AS "t14"