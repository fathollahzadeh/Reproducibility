...
FROM "posts"
WHERE "viewcount" >= 0 AND "answercount" <= 5 AND "commentcount" <= 12 AND "favoritecount" >= 0
GROUP BY "id"
HAVING "id" = "$cor5"."postid") AS "t3") AS "$cor4",
LATERAL (SELECT "relatedpostid", COUNT(*) AS "EXPR$0"
FROM "postlinks"
WHERE "linktypeid" = 1 AND ("creationdate" >= TIMESTAMP '2011-02-16 20:04:50' AND "creationdate" <= TIMESTAMP '2014-09-01 16:48:04')
...
LATERAL (SELECT "postid", COUNT(*) AS "EXPR$0"
FROM "votes"
WHERE CAST("creationdate" AS TIMESTAMP(0)) >= TIMESTAMP '2010-07-19 00:00:00' AND CAST("creationdate" AS TIMESTAMP(0)) <= TIMESTAMP '2014-08-31 00:00:00'
GROUP BY "postid"
HAVING "$cor2"."id0" = "postid") AS "t14"
GROUP BY "$cor2"."userid") AS "$cor1",
LATERAL (SELECT "userid", COUNT(*) AS "EXPR$0"
FROM "badges"
WHERE "date" >= TIMESTAMP '2010-08-06 10:36:45' AND "date" <= TIMESTAMP '2014-09-12 07:19:35'
GROUP BY "userid"
