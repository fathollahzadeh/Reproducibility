SELECT MIN("t1"."name") AS "character_name", MIN("t5"."info") AS "rating", MIN("name"."name") AS "playing_actor", MIN("t6"."title") AS "complete_hero_movie"
FROM "complete_cast"
INNER JOIN (SELECT *
FROM "comp_cast_type"
...
INNER JOIN (SELECT *
FROM "title"
WHERE "production_year" > 2000) AS "t6" ON "t4"."id" = "t6"."kind_id" AND "movie_keyword"."movie_id" = "t6"."id"